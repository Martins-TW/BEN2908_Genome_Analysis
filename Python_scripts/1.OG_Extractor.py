...
with open('address/address.txt','r') as single_ortho: #change address
    for og in single_ortho:
        og = og.split('\n')
        OGs.append(og[0])

with open('address/address.csv','r') as Ortho: #change address
        ...
        if val_cepa == 448865:
            with open ('APECO1.faa','r') as apeco1:
                for record in SeqIO.parse(apeco1, "fasta"):
                    if figpeg == record.id:
                        with open ('address/' + num_sog + '.fasta', 'a') as file_sog: #change address
            ...
            with open ('78-Pyelo.faa','r') as _78pyelo:
